...
## 進行中
- **Phase 7:全端化(React 前端 + 容器化 demo)** —— Stage 1–5(骨架 / 主檔 / 儀表板財報 / 採購 P2P / 銷售 O2C)已完成。接著:Stage 6 製造 → Stage 7 進階(調整/手動分錄/期間結)→ Stage 8 容器化(後端+前端 Dockerfile、nginx 反代、`compose.demo.yaml`)+ screenshots + README 收尾。每 stage 一個 PR(merge 等確認)。
