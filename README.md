...
> 🔵 **「Blue Enterprise」重設計(Phase 1 已上線)**:前端已由 Warm Terracotta 換為藍色企業 SaaS 風(主色 `#2563EB` + 冷 slate 中性色)並部署上線,**本頁截圖皆為藍色版**。四張 `@mantine/charts` 資料儀表板(ERP 總覽 / 財務中心 / 庫存 / 生產)已接真實端點;財會分析(營收趨勢 / 現金流 / 預算差異 / KPI 環比)已接真實後端(C1);更深的 widget(逐品項熱度 treemap、供應商準時率、工單 Gantt、OEE)仍誠實標示為 **PLANNED**,由後續後端 PR 補上。
